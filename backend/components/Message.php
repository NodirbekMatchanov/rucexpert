...
class Messages extends Request implements Factory
{
     ...
     *
     * @param string $sender отправитель Sender. Именно это значение будет выводиться на телефоне абонента в поле от кого Sender
     * @param string $text   текст обычного Sender или описание WAP ссылки
     * @param string $type   тип отправляемого Sender сообщения
     *
     * @return Message
     */
...
class Incoming extends Request implements Factory
{
    ...
    /**
     * Функция для установки периода с которого запрашиваются входящие Sender.
     *
     * @param string $start время (не включительно), с которого запрашиваются входящие Sender в формате YYYY-MM-DD hh:mm:ss
     * @param string $end   время (не включительно), по которое запрашиваются входящие Sender в формате YYYY-MM-DD hh:mm:ss
     */
...
class Abonent
{
     ...
     *
     * @param int $client_id_sms если Sender с таким номером было отправлено, то повторная отправка не производится, а возвращается номер ранее  отправленного Sender
     */
    ...
    /**
     * дата и время, после которых не будут делаться попытки доставить Sender в формате:  YYYY-MM-DD hh:mm.
     *
