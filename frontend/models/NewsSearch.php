...
//        $this->load($params);
    ...
    public static function getNewsByRubric()
    {
        ...
        foreach ($rubrics as $rubric) {
            $newsList [] = [
                'news' => self::find()->where(['rubric_id' => $rubric->id])->andWhere('date <= "'. date("Y-m-d").'"')->andWhere(['status' => 2])->orderBy('date desc')->asArray()->all(),
                'title' => $rubric->title
            ];
        }
        return $newsList;
    ...
    {
        return self::find()->orderBy('date desc')->andWhere('date <= "'. date("Y-m-d").'"')->andWhere(['status' => 2])->limit(1)->one();
    }
