    ...
    */
    ...
    $('#w5').bootstrapWizard({
        ...
        onNext: function (tab, navigation, index, newindex) {
            ...
            let valid = true;
            if (reg_hotel[0].checked || reg_car_type[0].checked || reg_rent_type[0].checked) {
                $('.step1_error').html('');
            } else {
                $('.step1_error').html('<span style="color: red">Выберайте вид деятельности</span>');
                return false;
            }
            if(index == 2){
                let company_name = $('input[name="SignupForm[company]"]');
                let email = $('input[name="SignupForm[email]"]');
                if (email.val() == '') {
                    email.closest('.field-signupform-email').find('.help-block').html('<span style="color: red">Необходимо заполнить «email».</span>');
                    valid = false;
                } else {
                    email.closest('.field-signupform-email').find('.help-block').html('');
                }
                if (company_name.val() == '') {
                    company_name.closest('.field-signupform-company').find('.help-block').html('<span style="color: red">Необходимо заполнить «НАЗВАНИЕ ОРГАНИЗАЦИИ».</span>');
                    valid = false;
                } else {
                    company_name.closest('.field-signupform-company').find('.help-block').html('');
                }
                if(!valid){
                    return false;
                }
